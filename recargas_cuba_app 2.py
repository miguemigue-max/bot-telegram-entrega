        ...
        <form method="post" enctype="multipart/form-data" id="depositWizardForm">
          ...
          <div class="wizard-step" id="step3" style="display:none;">
            ...
            <div style="margin-top:18px;display:flex;gap:12px;flex-wrap:wrap;">
              ...
              <button type="button" class="btn btn-primary" id="nextStep3">Ya hice el pago</button>
            </div>
          </div>
...
def admin_dashboard():
    ...
    <div class="page-wrap">
      <div class="container">
        ...
        <div class="panel" style="margin-top:20px;">
          <h3 style="margin:0 0 14px;">Usuarios</h3>
          <table>
            <thead>
              <tr>
                <th>ID</th>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td data-label="USD">{{ "%.2f"|format(u["usd_balance"] or 0) }}</td>
                <td data-label="Estado">
                  {% if u["is_admin"] %}
                    <span class="status status-aprobado">Admin</span>
                  {% elif u["is_suspended"] %}
                    <span class="status status-rechazado">Suspendida</span>
                  {% else %}
                    <span class="status status-aprobado">Activa</span>
                ...
                <td data-label="Acciones">
                  <div class="admin-actions">
                    {% if not u["is_admin"] %}
                      {% if u["is_suspended"] %}
                        <a class="btn btn-primary btn-sm" href="{{ url_for('admin_toggle_suspend_user', user_id=u['id']) }}">Activar</a>
                      {% else %}
                        <a class="btn btn-danger btn-sm" href="{{ url_for('admin_toggle_suspend_user', user_id=u['id']) }}">Suspender</a>
                      {% endif %}
                      <a class="btn btn-secondary btn-sm" href="{{ url_for('admin_notify_user', user_id=u['id']) }}">Notificar</a>
                    {% endif %}
                  </div>
                </td>
              </tr>
                ...
                <td data-label="Acciones">
                  <div class="admin-actions">
                    {% if w["status"] == "Pendiente" %}
                      ...
                      <a class="btn btn-secondary btn-sm" href="{{ url_for('admin_user_detail', user_id=u['id']) }}">Ver</a>
...
def approve_deposit(deposit_id):
    ...
    if deposit_currency == "USD":
        ...
        conn.close()
        flash("Moneda de depósito no válida.", "error")
        return redirect(url_for("admin_dashboard"))
