...
def can_debit_wallet(user_id, currency, amount):
    ...
    if direction == "BR_TO_CUBA":
        return parse_float(settings.get("remesa_brl_to_cup", "120"), 120)
    return parse_float(settings.get("remesa_cup_to_brl", "0.0083"), 0.0083)
    ...
    <div class="auth-shell">
      <div class="auth-card panel" style="max-width:760px;">
        ...
        {% else %}
          ...
          <form method="post">
            <button class="btn btn-secondary" type="submit" name="back" value="1">Atrás</button>
...
def remesas_page():
    ...
    if request.method == "POST":
        ...
        if direction == "BR_TO_CUBA":
            ...
            if not receiver_pix_key:
                flash("Escribe la llave PIX del destinatario.", "error")
                return redirect(url_for("remesas_page"))
        ...
        <form method="post" id="remesaWizardForm">
          ...
          <div class="wizard-step" id="rstep1">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;">
              <button type="button" class="direction-option btn btn-secondary" data-direction="BR_TO_CUBA">Brasil → Cuba</button>
              <button type="button" class="direction-option btn btn-secondary" data-direction="CUBA_TO_BR">Cuba → Brasil</button>
            </div>
          ...
          <div class="wizard-step" id="rstep2" style="display:none;">
            ...
            <div class="panel" style="margin-top:14px;background:#faf7fd;padding:18px;">
              <div><strong>Recibirán:</strong> <span id="receivePreview">0.00</span></div>
            </div>
    ...
    document.addEventListener("DOMContentLoaded", function () {
      ...
      function calculateReceive(direction, amount) {
        if (!direction || !amount || amount <= 0) return 0;
        if (direction === "BR_TO_CUBA") return amount * brlToCup;
        if (direction === "CUBA_TO_BR") return amount * cupToBrl;
        return 0;
      }
      ...
      document.querySelectorAll(".direction-option").forEach(btn => {
        btn.addEventListener("click", function () {
          ...
          document.getElementById("rnext1").disabled = false;
        });
      });
